...
public class Unfilter {
	...
	public Unfilter(int width, int bpp, byte[] data) {
		...
		for (int i = 0; i < data.length; i += (width + 1)) {
			...
			else if (filterType == 1) { // i.e. "Sub": sub(x) = raw(x) -
										// raw(x-bpp)
			...
			else if (filterType == 3) { // i.e. "Average": average(x) = raw(x) -
										// floor( ( raw(x-bpp)+prior(x) )/2 )
				for (int j = 1; j <= width; j++) {
					data[i + j] = (byte) (data[i + j] + (rawMinusBpp(i + j) + prior(i + j)) / 2);
				}
			} else if (filterType == 4) { // i.e. "Paeth": paeth(x) = raw(x) +
											// paethPredictor( raw(x-bpp),
											// prior(x), prior(x-bpp) )
				for (int j = 1; j <= width; j++) {
	...
	public int prior(int i) {
		if (i <= width)
			...
			return data[i - (width + 1)] & 0xFF;
	}

	public int priorMinusBpp(int i) {
		if (i % (width + 1) <= bpp || i <= width + 1)
			return 0;
		else {
